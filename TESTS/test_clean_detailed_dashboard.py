...
for yr in all_years:
    ...
    # Track trade share of key nations
    ...
    if yr == 2024:
        ...
        # Geopolitical flux per node vs Gravity flux per node
...
# Panel 4: Inferred Geopolitical Latent Space S in 2024 (Country Clusters)
ax4 = axes[1, 1]
...
for i in range(min(16, len(latest_df_top))):
    ax4.annotate(latest_df_top.loc[i, "iso"], (latest_S_hat[i, 0], latest_S_hat[i, 1]),
...
plt.tight_layout()
plt.savefig("test_clean_country_dashboard.png", dpi=150)
print("\nPlot saved successfully to test_clean_country_dashboard.png")
