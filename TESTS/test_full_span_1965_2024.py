...
# Years configuration: historical years from Gravity + recent yearly from BACI
...
print(f"Total years to analyze: {len(all_years)}")
print(f"Historical years (CEPII Gravity): {hist_years}")
print(f"Recent years (BACI HS17 2017-2024): {baci_years}")

# --- Load historical years from Gravity_V202211.csv in a single fast chunked pass ---
...
print(f"Loaded {len(df_hist)} historical rows in {time.time() - t_h:.2f}s.")

# --- Inference loop for all years ---
results = []
print("\n" + "="*85)
print("RUNNING PROFILE LIKELIHOOD INFERENCE: 1965 → 2024 (60 YEARS OF GLOBAL TRADE)")
print("="*85)
...
for yr in all_years:
    ...
    top_3_str = ", ".join([f"{iso_to_name.get(c, c)} ({c})" for c in list(top_series.index[:3])])
...
print("\n" + "="*95)
print("FINAL MULTI-DECADE RESULTS SUMMARY (1965–2024):")
print("="*95)
print(df_res[["year", "source", "lambda", "ci_lo", "ci_hi", "gravity_share", "r_km", "active_links", "runtime_s"]].to_string(index=False))

# ----------------------------------------------------------------------
# Visualization: 4-Panel Plot 1965 → 2024
# ----------------------------------------------------------------------
...
# Panel 1: Lambda vs Year (1965-2024)
ax1 = axes[0, 0]
...
plt.tight_layout()
plt.savefig("test_1965_2024_evolution_figure.png", dpi=150)
print("\nPlot saved successfully to test_1965_2024_evolution_figure.png")
